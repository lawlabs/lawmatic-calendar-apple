...
## Защита от массовой потери данных

На сервер (Google, Apple, LEGALIC) уходят только два вида операций — `pushUpsert`
и `pushDelete`, и обе питаются исключительно явными действиями пользователя над
одним событием: правка/перенос ставит `syncState = .pendingUpload`, удаление или
перенос в другой календарь кладёт один tombstone в `pending-event-deletions.json`.
Чтение ленты, merge, чистка «сирот», исчезнувшие календари, сбои загрузки — всё
это меняет только локальные копии и никогда не порождает удалений на сервере.

Дополнительные предохранители:

- **Порог массового удаления** (`CalendarSyncCoordinator.massDeletionThreshold`, 20):
  больше стольких tombstones для одного провайдера за прогон не отправляются —
  синк останавливается с вопросом «Удалить N / Отменить удаления». Отмена снимает
  tombstones, ничего не трогая на сервере; локальные копии вернёт следующее чтение.
- **LWW**: если серверная версия новее локальной правки, локальная не отправляется.
- **Apple Calendar**: тумблер «Разрешить изменять события Apple Calendar из
  приложения» (выключено — календари только читаются); экземпляры повторяющихся
  событий помечены `isReadOnly` и не редактируются и не удаляются из приложения;
  удаление затрагивает только `.thisEvent` и только если событие всё ещё лежит в
  том календаре, из которого удалено локально.
- **LEGALIC**: запись задач выключена по умолчанию; сроки по делам только читаются.
- Удаление в приложении обратимо через ⌘Z, пока tombstone не ушёл на сервер.
